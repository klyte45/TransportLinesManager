...
namespace Klyte.TransportLinesManager.Interfaces
{
    public abstract class ExtensionInterfaceDefaultImpl<T, U> : BasicExtensionInterface<T, U> where T : struct, IConvertible where U : ExtensionInterfaceDefaultImpl<T, U>
    {
        ...
        private ref Dictionary<uint, Dictionary<T, string>> GetDictionaryData(bool global)
        {
            ...
            { throw new Exception("CONFIGURAÇÂO NÃO GLOBAL TENTOU SER UTILIZADA COMO GLOBAL: " + typeof(U)); }
            if (global)
            {
        ...
        private void Load(bool global = false) => GetDictionaryData(global) = LoadConfig(ConfigIndexKey, global);

        private void Save(bool global = false) => SaveConfig(GetDictionaryData(global), ConfigIndexKey, global);

        #region Utils R/W
        protected string SafeGet(uint idx, T key, bool global = false)
        {
            Dictionary<uint, Dictionary<T, string>> cachedValues = GetDictionaryData(global);
            if (cachedValues == null)
            {
                Load(global);
                cachedValues = GetDictionaryData(global);
            }
            if (!cachedValues.ContainsKey(idx) || !cachedValues[idx].ContainsKey(key))
            {
                return null;
            }
