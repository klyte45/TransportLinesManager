...
namespace Klyte.TransportLinesManager.CommonsWindow
{
    internal abstract class TLMTabControllerDepotHooks<T, V> : MonoBehaviour, IRedirectable where T : TLMTabControllerDepotHooks<T, V> where V : TLMSysDef<V>
    {
        ...
        public void Awake()
        {
            System.Reflection.MethodInfo from = typeof(BuildingManager).GetMethod("CreateBuilding", allFlags);
            System.Reflection.MethodInfo to = typeof(TLMTabControllerDepotHooks<T, V>).GetMethod("AfterCreateBuilding", allFlags);
            System.Reflection.MethodInfo from2 = typeof(BuildingManager).GetMethod("ReleaseBuilding", allFlags);
            System.Reflection.MethodInfo to2b = typeof(TLMTabControllerDepotHooks<T, V>).GetMethod("BeforeReleaseBuilding", allFlags);
            System.Reflection.MethodInfo to2a = typeof(TLMTabControllerDepotHooks<T, V>).GetMethod("AfterReleaseBuilding", allFlags);
    ...
    internal abstract class TLMTabControllerDepotList<T> : TLMTabControllerListBase<T> where T : TLMSysDef<T>
    {
        ...
        protected override void CreateTitleRow(out UIPanel titleLine, UIComponent parent)
        {
            ...
            KlyteMonoUtils.CreateUIElement(out UILabel districtTitle, titleLine.transform, "districtTitle");
            districtTitle.minimumSize = new Vector2(140, 0);
            districtTitle.area = new Vector4(00, 10, districtTitle.minimumSize.x, 18);
            KlyteMonoUtils.LimitWidth(districtTitle, (uint) districtTitle.width);
            ...
            KlyteMonoUtils.LimitWidth(prefixesServed, (uint) prefixesServed.width);
        ...
        private static Type ImplClassChildren => ReflectionUtils.GetImplementationForGenericType(typeof(TLMDepotListItem<>), typeof(T));
        ...
        public override void RefreshLines()
        {
            ...
            {
                case DepotSortCriterion.NAME:
                    OnNameSort();
                    break;
                case DepotSortCriterion.DISTRICT:
                    OnDistrictSort();
                    break;
            }
            isUpdated = true;
